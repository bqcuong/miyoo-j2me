...
void musicFinished() {
    printf("%s musicFinished\n", name);

    Mix_HookMusicFinished(NULL);

//    jstring message = aEnv->NewStringUTF("Register callback successfully!");
//    aEnv->CallVoidMethod(m_jobj, m_onCallBack, message);
...
JNIEXPORT void JNICALL Java_org_recompile_mobile_PlatformPlayer__1start
    (JNIEnv * aEnv, jobject object, jstring aBgm, jint loop) {
        ...
        if (m_onCallBack == NULL) {
            ...
            m_onCallBack = aEnv -> GetMethodID(m_jclz, "onCallback", "(Ljava/lang/String;)V");
            jstring message = aEnv->NewStringUTF("Register callback successfully!");
            aEnv->CallVoidMethod(m_jobj, m_onCallBack, message);
        }
